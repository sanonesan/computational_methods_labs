...
void templated_2_layer_difference_scheme(const Class_1d_heat_equation<T>& heat_equation, const T sigma, const std::string& out_path) {
    ...
    y[y.size() - 1] = heat_equation._boundary_conditions[0](x[x.size() - 1], time[0]);
    fout << "," << y[y.size() - 1] << "\n";
    ...
    for (std::size_t j = 1; j < time.size(); ++j) {
        ...
        if (heat_equation._left_boundary_condition_type == 1) {
            kappa = c_rho_h_tau / 2 * y[0];
            kappa += sigma * heat_equation._boundary_conditions[0](x[0], time[j] + heat_equation._tau);
            a_i = (heat_equation._K(y[1], x[1]) + heat_equation._K(y[0], x[0])) / 2;
            kappa += (1 - sigma) * (heat_equation._boundary_conditions[0](x[0], time[j]) - a_i * (y[1] - y[0]) / heat_equation._h);
            kappa /= c_rho_h_tau / 2 + sigma * a_i / heat_equation._h;
            banded_matrix[1][1] = -kappa;
            
            mu = sigma * a_i / heat_equation._h;
            mu /= c_rho_h_tau / 2 + sigma * a_i / heat_equation._h;
            b[0] /= mu;        
        }
        ...
        if (heat_equation._right_boundary_condition_type == 1) {
            ...
            kappa = c_rho_h_tau / 2 * y[y.size() - 1];
            kappa += sigma * heat_equation._boundary_conditions[1](x[x.size() - 1], time[j] + heat_equation._tau);

            a_i = (heat_equation._K(y[y.size() - 1], x[y.size() - 1]) + heat_equation._K(y[y.size() - 2], x[y.size() - 2])) / 2;

            kappa += (1 - sigma) * (heat_equation._boundary_conditions[0](x[x.size() - 1], time[j]) - a_i * (y[y.size() - 1] - y[y.size() - 2]) / heat_equation._h);
            kappa /= c_rho_h_tau / 2 + sigma * a_i / heat_equation._h;
            banded_matrix[1][banded_matrix[1].size() - 2] = -kappa;
