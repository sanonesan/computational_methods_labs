...
double *read_vec(string path, size_t &n)
{
    ifstream file;
    file.open(path);
...
double** read_Sys_MatrixFromFile(string path, size_t& n) {
...
//--------//
int e_norm(double*& v, double& norm, size_t& n) {
...
int LDU_method(double**& A, double**& L, double**& D, double**& U, size_t& n){
	...
	for (size_t i = 0; i< n; ++i){
		...
		for (size_t j = 0; j < n; ++j){
			...
			L[i][j] = (i < j + 1) ? 0.0 : A[i][j];
			U[i][j] = (i + 1 > j ) ? 0.0 : A[i][j];
			D[i][j] = (i != j ) ? 0.0 : A[i][j];
